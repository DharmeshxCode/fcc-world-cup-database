...
do
  ...
  then
    ...
    # Check if the winning team is already in the database
    WINNER_ID=$($PSQL "SELECT team_id FROM teams WHERE name='$WINNER'")
    
    # If not found, insert the winning team
    if [[ -z $WINNER_ID ]]
    then
    ...
    # Check if the opposing team is already in the database
    OPPONENT_ID=$($PSQL "SELECT team_id FROM teams WHERE name='$OPPONENT'")
    
    # If not found, insert the opposing team
    if [[ -z $OPPONENT_ID ]]
    then
    ...
    # Insert the current game record using the retrieved IDs
